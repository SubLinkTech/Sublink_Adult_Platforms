...
if (Test-Path builds\SubLink_Adult-$version-linux-x64.zip) {
    Remove-Item builds\SubLink_Adult-$version-linux-x64.zip;
}

Compress-Archive -Path "build-$version\*" -destinationpath "builds\SubLink_Adult-$version-linux-x64.zip" -compressionlevel optimal;
